from board import Vrhcaby_board
...
class Vrhcaby_game(Vrhcaby_board):
	...
	def __init__(self, start_board=start_board):
		...
		super().__init__(start_board, white_to_move) #smer pohybu kamenu
		print("""Bílý:      Černý:
->->|              <-<-^
<-<-ˇ              ->->|
""")     
	...
	def roll_and_parse_dice(self):
		dice = roll_dice()
		print("Hodil: ", dice)
		if dice[0] == dice[1]: # stejné dvojice, 4 tahy
			self.dice = [dice[0]] * 4
		else:
			self.dice = list(dice)
